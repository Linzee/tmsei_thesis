...
In this chapter we will talk in general about questions in tutoring
...
of the thesis more in detail.

\section{Tutoring systems}\label{tutoring-systems}

% Tutoring systems

Tutoring systems are computer systems which purpose is to teach its users (students) some knowledge or skill. This systems do this autonomously to some degree. This degree may vary for each tutoring system.

%%% Autonomous aspects of tutoring systems

Many different aspects of learning may be implemented into the system or they may also be leaved out. Most common task which tutoring systems solve is choosing most beneficial item from pool of items for users to solve. This can be done in some straightforward way or we can adapt choices to each of the users. Choosing more challenging and interesting items to users who are skilled in specific area and more basic for users who are not is beneficial for maximizing learning aspect of the system. Another area is providing hints and feedback to students having problems while solving items or after failing to solve them correctly. When there are teachers interacting with the system we can also provide feedback about their students progress and so on.

%% Recommender systems

One closely related area are recommender systems which differs from tutoring systems only slightly. Both areas are observing users and items. Main difference is that our main source of information is performance of users while solving specific item and recommender systems mostly use rating of the items. Which in fact are still only numbers so we can use same techniques. However different goals bring different problems specific to each area. 
...
This description of item is broad enough to cover most of tutoring systems. End of this chapter discuses more closely tutoring system which is used for our results.

%%% Tutoring systems contains educational items, Large pool is required in some contexts

On of the most important property of tutoring systems is that they contain educational items which are solved by users. Amount of items in this systems may vary. Some tutoring systems require large amount of items. This is especially true when they practice simple facts instead of skills. In personalized tutoring systems the need for large pool of items is even higher. For providing personalized questions we need wider set of items to choose from.

%%% May be useful to determine similarity of items

And dealing with large pool of items may not be easy. To maintain pool of items efficiently we need to be able to decide which items are useful and which are not. One possible tool which can help us here is similarity of items. 
...
As we mentioned previously key part of learning is solving of educational items. Defining some metrics for measuring similarity of items can then be used different purposes.

First, most direct, usage is recommendation of items for student to solve. We do not want to recommend very similar items to those that were solved without any problems. However when user struggled system should consider recommending more of the similar problems to strengthen users knowledge.

Another possible usage is generating hints by selecting examples which are similar to item which is solved. Examples are selected from database of examples. Computation of similarity was used by [CITE Hosseini; Brusilovsky] for this purpose.

Two previous use cases were using problem similarity to automatically make some choices inside tutoring system. Another approach is to bringing humans into the decision-making loop [CITE]. This approach provides authors of tutoring system with visualizations which should inform them what
changes may be useful. E.g. detecting possibly redundant problems or pointing out where there is not enough similar problems to choose from user struggle.

One possible way of achieving this is plotting problems to plane and displaying it to author. This still ca not be used for very large amount of items. But we choose this approach for our specific data as they contain large amount of items but can be divided into item-sets which are solved independently in the system.

One of the use cases of visualizations is detection of outsiders - items which behave differently than others. This behavior is directly shown in projection and such items lie far away from others. 

There is one ore usage of similarity which we wont be discussing. Similarity of items can be utilized for automatic construction of clustering and hierarchical categorization. When author already has items categorized he can compare it to computed categorization to verify that groups
are formed correctly and refine them if needed.
...
are useful for concluding any practical results. However evaluation of this data
...
Primary group of users using Umíme česky are children children aged ? to ?. System also support teachers and usage in school. Each week about 200 classes visit the system.

% TODO basic statistics of users
