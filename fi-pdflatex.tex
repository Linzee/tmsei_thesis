...
\documentclass[
  print, %digital, %% This option enables the default options for the
...
\thesissetup{
    ...
    abstract      = {Work focuses on measuring of item similarity in tutoring systems using correctness of answers from users. We knew that some unexplained regularities might appear in a similarity of items. In observed tutoring system they caused separation of items into surprising clusters. Core of the work consists of an exploitative analysis of possible causes. We found multiple high-level factors affecting similarity of items and therefore its clusters and projections. We show that structure of the system can affect item similarities. E. g.,  answer preferred by users causing separation of items by correct answer or pattern of missing data. We validated these conclusions using simulations. Findings are useful for further usage in the analyzed system or replicating similar experiments in other tutoring systems and context.},
    thanks        = {First, I would like to thank my advisor \radek{} for providing me the opportunity to work in the Adaptive Learning group. I want to thank him for giving the right pointers and asking the right questions. I appreciate his expertise and his time he was always willing to find for me.

AL group

My further thanks go to my family and friends, who were very accommodating of my needs and supported me during my work.
...
This short chapter summarizes recommendations gathered after looking at results of our experiments. We list both general recommendations which I think are useful for everyone and practical recommendations for future work on this specific tutoring system.
...
\begin{table}
  \begin{tabular}{ | l | l | }
      \hline
      \cellcolor[gray]{1.0} & Using one answer by default                 \\
      \hline
...
Table \ref{tab:effect-of-factors-on-stages} summarizes how described high-level factors affect each stage of similarity pipeline. When we use similarity for computation of projection in at the last stage they all cause some clusters. However, they all differ at how they affect stages before last stage. For example in similarity matrix two of observed factors cause higher similarity of some group of items while the factor of pattern in missing data only affects stability of values.

It is worth knowing how high-level factor can affect each stage of similarity pipeline. Factor of missing answers does not affect outlier detection techniques which utilize only similarity of items. (Only if technique compares them to each other, this factor affects the result.)
...
Adaptive learning group currently focuses on tutoring systems for teaching introductory programming.
some of the findings beneficial
