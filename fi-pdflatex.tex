...
describes structure of exercices and items intutoring system Umíme česky
...
So when simulating data we are trying to generate results which are similar to real tutoring system. Hovewer, we use slightly simpler model.
...
The primary group of users using Umíme česky are children stuing at primary and secondary school. The system supports accounts for inidviduals as well as accounts arranged into classes with teachers assigned. Each week about 200 classes visit the system.

% TODO translate
Učitelé mají možnost založit virtuální třídu, do které mohou při-
dat své žáky a přiřadit jim domácí úkoly k vypracování. Učitel může
vybrat několik cvičení k vypracování a nastavit datum, do kterého
má být úkol vypracován. Po vypracování může splnění úkolu jedno-
duše zkontrolovat, případně i zjistit, jaké výrazy žákům dělají největší
potíže.
...
% Similarity measure
Most of the work relevant to our problem comes from article Measuring Similarity of Educational Items Using Data on
Learners’ Performance [CITE]. 
It was determined which similarity measures it is best to use for stability of results
compare similarity measures to each other
some metrics correlate more that other
using Pearson is good default choice

There was an article focused on comparing similarity measures. how well they preserve clusters on simulated data and whether they correalte on real datasets.
Some of the measures correlate and some does not.
...
projections}\label{visible-properties-of-projections}
...
incorporate user solving time into this value [CITE]. Performance matrix is relatively
...
Next step is computing \textbf{similarity matrix}. It is filled with pairwise correlation of all items from performance. We choose Pearson correlation coefficient for this and it was used for several reasons. Previous work showed that it has good enough results in producing distinct clusters on performance data [CITE]. Same article also showed that using second level improves this property even further. However, we choose not to use it to simplify explaining our experiments as that is important factor to our work. Another reason is that Pearson correlation is easy to use as many computational environments already contain fast implementation.
