...
\begin{figure}
  ...
  \caption{Basic projection of item-set \cvicenieB{}. Three used colors distinguish levels and markers correct answers of items. The highlighted group (items inside circle) is words starting with prefix ``bio'' which are correctly considered similar and appear close to each other. On the other hand, two connected items should be considered similar, but they are not because they are in different levels.}
