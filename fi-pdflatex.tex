...
It is important to choose appropriate pipeline for calculating similarity. One we experimented with is good choice. Because %TODO why?. However, there are some unneeded regularities we have to deal with.

For developers of system it is worth knowing that structure of user interface can affect data in some unexpected way (refer to section \ref{regularities}). In case that similarity would be used extensively it may be useful to implement some changes to user interface to prevent such regularities.

% different pipeline - small amount of answers

Another possible solution is to normalize data in some way when processing them. For example because item-sets with large number of correct answers are not affected by preferred answers we ca use some modified pipeline only for item-sets with small amount of answers.

% similarity measure choice

We did not find any difference in usability of observed similarity measures, therefore we recommend to use any good performing and convenient to use. Based on previous research this can be Pearson correlation coefficient applied two times sequentially. This measure has good stability of results even on smaller datasets and is easy to implement.

% in visualizations

Most straightforward visualization of item similarity are projections. Visualizing some known property like belongingness to level or place of item in taxonomy can be used to verify whether it is good or may need some refinement. Another possible use is interactively displaying single item-set and observing particular items manually.

% clustering

Projections and clustering usually suffer from same problems. E. g., Pattern of missing answers affects both of them. This cannot be easily solved. But as this regularity is caused by unstability of items similarity it can be improved by using more stable similarity measure.

But projections and clustering are also affected by formation of clusters of same items with same correct answer. When using them we have to be aware of this.

% in outlier detection

For outlier detection we can recommend using method that does not depend on global distribution of values, only on local neighborhood as items with some answers have higher similarity. Another possibility is to normalize data in some way. One example is proposed removal of users with great difference of answer preference. But other less drastic normalization method can also be found we only used this one to show that this users are a cause for this separation of items by correct answer.

% does not matter

We also found multiple properties that does not affect anything. Different performance of items in levels does not cause clusters, ratio of correct answers in item-set does not affect similarity. %TODO something more?
