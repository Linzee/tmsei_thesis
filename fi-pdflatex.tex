...
\hypersetup{colorlinks, urlcolor=[rgb]{0.6,0.2,1.0}}
\lstset{
  basicstyle      = \ttfamily,%
...
% \newcommand{\umimeCesky}{\textcolor[rgb]{0.6,0.2,1.0}{Umíme česky}}
...
% post-summary of this work

This work consists mostly of exploitative analysis of similarity measures. At the beginning we knew about existence of some regularities in data that caused clusters of items that were same in some way that should not be present in the data. On a way for explaining them we found multiple high-level factors that can possibly affect results of calculated similarity and projections of items in tutoring systems. We also encountered some problems and therefore can give some recommendations for future work in this or comparable area.

% Reusable in other systems %TODO move
...
I think it is always good idea to first determining range of effect. E. g., whether this problem also appears when we use some different measure or data. I know it is basic. But it can be easily forgotten when facing some specific problem. And it was in great help to us. Such experiments were explained in first section \ref{evaluation-basics} of previous chapter.

% simulations

%TODO
- we are using unsupervsed learning like techniques - we obtain some results but it is
hard to explain why they are like they are.. one possibility how to
explain results is using simulations

% more dimensions of PCA

%TODO
when using PCA.. look at more than first two dimensions may be useful
- looking at total similarity of items - typicaly correlates with first
dimension of

% > ???

\section{To co ma realne obsahovat zaver}\label{to}

Now we understand how structure of system may affect performance data and therefore similarity of items itself.
...
In the work we analyzed in detail similarity measures and their use cases on dataset from \umimeCesky{}. Especialy we focused on using data about user performance from single exercise.
