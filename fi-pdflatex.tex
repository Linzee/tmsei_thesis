...
There is a lot of research about similarity in many fields bioinformatics (sequence alignment, similarity matrix of proteins), data >>>processing/retrieval<<< (document similarity), plagiarism detection and many more. But only a few mentions about comparing programming problems are currently published. When we talk about programming problems we in most cases mean problems created for interactive programming environments for introductory programming. Some examples of environements are [Robotanik](https://tutor.fi.muni.cz/), []>>>robot z modling how students learn to program<<<](http://example.com), [Lightbot]().

Problems are specified by grid with tiles which have different meaning. Users are suppose to write program for >>>entity/character<<< to fullfill some task. Most common task is to visit all tiles of some type. For example collect all flowers in Robotanik. Sometimes problems also limit >>>user/learner<<< to subset of all available commands or length of program is limited.

>>>Closest = Similarity Techniques for Detecting Source Code Plagiarism
...
When we define metrics for measuring similarity of programming problems we can use them for different purposes.

recommendations: do not recommend very similar problems to those that were solved without problems, do recommend similar problems if student struggled

hints

user interface (nezoradene problemy, vytvorit nejake zoradenie ktore vizera rozumne)

"management" of problem pool (redundant problems, missing problems, …)
	plot to plane
...
We can define different metrics for comparing programming problems. It's possible to divide metrics into 3 categories based on data that is used.
...
*Solution syntax* metrics differ greatly with method used by users to write solutions - programs. For programming environments using >>>standard<<< programming languages like Java metrics comparing preprocessed source-code and analysis of solution abstract syntax tree (AST) can be used. In both methods we can compute similarity as edit distance (>>>LLLL edit distance<<< and Tree edit distance respectively) or presence of keywords, their count>>>,..<<<
Some programming environment doesn't support text as solution but some simpler alternative is provided for learners. For example Robotanik uses commands (blocks) that can be placed in rows representing functions. Robomise >>>and Scratch<<< uses >>>NESTED DROP AND FILL<<< interface. It is usually simpler to convert solutions into some string or tree which can be used with standard methods.
Metrics can also make use of multiple solutions collected from users.

*Performance based* metrics requires having collected data about students while solving problems. Example of data that falls in this category is solution time, number of attempts, hints used,
With this type of metrics we compute similarity as correlation of performance on two problems.

\section{(Scicentific) Question, Goal}
...
2. Use "human data" (expert/student judgement) experiment -- collect judgement about (dis)similarity of items and compare with automatic metrics.
...
Robotanik (tutor.fi.muni.cz) problems
...
\subsection{Compare metrics}
compare metrics

\subsection{Collecting expert data}
collect expert options
	methods, triplets, scale
    Rand index
