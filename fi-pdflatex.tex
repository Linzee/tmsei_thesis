...
Chapter is explaining factors that can affect resulting projections produced from real-world data. We are focusing on higher level factors that can cause formation of clusters in projection. On low level it is apparent that items which correlate will be similar (close to each other) in projection but we focus on higher-level factors in tutoring systems that can cause this correlation.
...
Our performance matrix contains only one value for each item-user pair. But users can possibly answer to the item multiple times. This raises a question which of user answers to use. [CITE]

% What we used in performance matrix

We tried using both first users answer and last in performance matrix. However there is no visible difference in results. This is clear when we compare first and last answers of users. There is only 5\% different answers between them. So at least with large number of items in system it does not really matter which one is chosen as only few items are answered multiple times.

\subsection{Quality of clusters on different item-sets}\label{quality-of-clusters}

When introducing problem of unneeded clusters in section \ref{regularities} we showed that one particular item-set shows this behaviuor. However, we still do not know whether this is true for all the item-sets. We decided it may be useful to quantify how recognizable are this clusters on all item-sets.
...
% is this too much users removed?

When we use only half of users which performance is more uniform separation of items with same answers is gone. Its half of users because we compare their difference to median difference. Apparent question is whether smaller amount is of removed user may not be enough. % TODO more new text here

%TODO ??? We look at performance of items instead of usage of each answer because it is performance that directly affects calculated similarity of items. However they should correlate somewhat. Another reason is that it is closer to simulated experiment so we can compare results directly.
...
So how have this happen? TODO find good explanation :D I do not have any with which I am 100\% happy with right now.

% > > Item performance

\subsection{Item performance}\label{item-performance}

As we know items are divided into in such manner that difficulty of levels raises. Question is whether this is directly projected into similarity of items. The answer is no when we use Pearson correlation to calculate similarity. However, some other measure like Jaccard can be affected.

%TODO can they?

Figure\ref{fig:item_performance_levels} shows mean performance of items. Horizontal axis contains items sorted by level and mean performance. Vertical axis shows performance of each item. Given item set contains three item-sets with mean performance 94\%, 86\%, 71\% respectively.
...
\subsection{Different groups of users}\label{different-groups-of-users}

We can look at previous figure \ref{fig:item_performance_levels} little differently. After dividing users into 8 groups based on levels they solved (according to table \ref{tab:user-groups}) plot changes slightly. Users in group who solved only first level (red) has much lower performance on other levels. We can conclude that users who solve primarily first level are not as experienced as other users. On other hand users in group who solved both 2nd and 3rd level (cyan) are performing better than other users on all three levels.
...
Instead we choose to analyse whether our data contains different groups
