...
We picked few item-sets which have very distinct clusters of answers and looked whether the answer with higher similarity is located on left or on right in user interface. Our finding was that there are item-set with preferred answer on both left and right button. In particular in concept ``Vyjmenovaná slova'' which have possible answers ``i'' (left) and ``y'' (right) is preferred left answer. But for concept ``Koncovky přídavných jmen'' which has same possible answers is preferred one ``y'' on the right.
...
When we look at resulting projection \ref{fig:simulated_default} of simulated performance data we can see that it has same properties as our real data. There are clusters (in this case caused by uncorrelated skills) which have items separated by correct answer (represented by different colors).
...
Figure\ref{fig:item_performance_levels} shows mean performance of items form one item-set. Horizontal axis contains items sorted by level and mean their performance. Vertical axis shows performance of each item. Given item set contains three item-sets with mean performance 94\%, 86\%, 71\% respectively.
